		...
		return context.getProgramInfo(function(err, info) {
			...
			info.program.events.listen[eventId].forEach(function(handler) {
				...
				return waitfor(function(done) {
		            return context.vm.loadPackage(packageInfo.dirpath, {
						rootModule: handler.handler.substring(packageInfo.dirpath.length + 1)
					}, function(err, sandbox) {
		                if (err) return done(err);

			            return exports.context(context.env.PINF_PROGRAM, PATH.join(packageInfo.dirpath, "package.json"), {
			            }, function(err, ctx) {
			                if (err) return done(err);

			                return sandbox.main(ctx, {
			                	event: eventId
			                }, function(err, result) {
			                    if (err) return done(err);
	...
	try {
		...
		function loadConfigs(callback) {
			...
			return PROGRAM_INSIGHT.parse(PATH.dirname(env.PINF_PROGRAM), opts, function(err, descriptor) {
	            ...
	            if (!packageDescriptorPath && !env.PINF_PACKAGE) {
	            	if (descriptor.combined.boot && descriptor.combined.boot.package) {
	            		packageDescriptorPath = PATH.join(descriptor.combined.boot.package, "package.json");
	            	}
	            }
