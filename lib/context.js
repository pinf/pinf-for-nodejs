...
console.log("TODO: Call `context.config.open` to open program.");
	...
	Context.prototype.bundleProgram = function(callback) {
		var self = this;
		var vm = new VM(self);
		...
		opts.distPath = PATH.join(self.paths.program, (
		...
		for (var uri in bundles) {
			waitfor(uri, function(uri, done) {
		        ...
		        return vm.loadPackage(bootPackageInfo.dirpath, opts, function(err, info) {
		        	if (err) return done(err);
