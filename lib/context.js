...
exports.context = function(programDescriptorPath, packageDescriptorPath, options, callback) {
	...
	var context = {
		...
		lookupPaths: [],
		_descriptor: {},
		makePath: function(type, path) {
			return ensureParentPath(context.paths[type], path);
		},
		ensureRuntimeConfig: function(ns, defaultConfig, overrideConfig, options, callback) {
			if (typeof overrideConfig === "function" && typeof options === "undefined" && typeof callback === "undefined") {
			...
			if (!context.uid) {
				return callback(new Error("`uid` must be set for package '" + context.paths.package + "' if wanting to read/set runtime configuration."));
			}
			options = options || {};
			if (!Array.isArray(ns)) {
				ns = [ ns ];
			...
			try {
				var store = new JSON_FILE_STORE(context.env.PINF_RUNTIME);
				if (!store.exists()) {
					store.init();
				}
				var key = ["config", context.uid].concat(ns);
				var data = store.get(key) || {};
				if (defaultConfig) {
					data = DEEPMERGE(defaultConfig, data);
					// TODO: Write default config to `/.program.json` file.
				}
				if (overrideConfig) {
					data = DEEPMERGE(data, overrideConfig);
				}
