...
*/
	...
	options.onInitPackage = function(pkg, sandbox, options) {
		...
		pkg.require = function(moduleIdentifier) {
			...
			if (sandboxOptions.debug) console.log("[loader-for-nodejs][pkg.require]", "moduleIdentifier", moduleIdentifier);
			...
			if (origModuleIdentifier) {
				...
				if (sandboxOptions.debug) console.log("[loader-for-nodejs][pkg.require]", "moduleIdentifier", moduleIdentifier, "due to", "origModuleIdentifier");
            	if (!/^\//.test(moduleIdentifier)) {
	                moduleIdentifier = "/" + ((moduleIdentifier.substring(0, pkg.libPath.length)===pkg.libPath)?"":pkg.libPath) + moduleIdentifier;
					if (sandboxOptions.debug) console.log("[loader-for-nodejs][pkg.require]", "moduleIdentifier", moduleIdentifier, "due to", "!/^\//.test(moduleIdentifier)");
	            }
				canonicalId = pkg.id + moduleIdentifier;
				if (sandboxOptions.debug) console.log("[loader-for-nodejs][pkg.require]", "canonicalId", canonicalId, "due to", "origModuleIdentifier");
			} else
			if (pkg.descriptor && pkg.descriptor.main) {
				canonicalId = moduleIdentifier = pkg.descriptor.main;
				if (sandboxOptions.debug) console.log("[loader-for-nodejs][pkg.require]", "canonicalId", canonicalId, "due to", "pkg.descriptor && pkg.descriptor.main");
			} else {
				moduleIdentifier = "";
				canonicalId = pkg.id;
				if (sandboxOptions.debug) console.log("[loader-for-nodejs][pkg.require]", "canonicalId", canonicalId, "due to", "fallback");
			}


			if (
				options.initializedModules[canonicalId] ||
				options.moduleInitializers[canonicalId]
			) {
				return origRequire(origModuleIdentifier);
			}
			if (
				options.initializedModules[canonicalId.replace(/\.js$/, "/index.js")] ||
				options.moduleInitializers[canonicalId.replace(/\.js$/, "/index.js")]
			) {
				return origRequire(origModuleIdentifier.replace(/\.js$/, "/index.js"));
			}
...
*/
	...
	// TODO: Determine this based on requirements declared in bundle or bundle descriptor.
	if (sandboxOptions.includeBrowserGlobals === true) {
